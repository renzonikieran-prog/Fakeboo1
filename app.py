...
st.markdown("""
    ...
    .stat-box { display: inline-block; width: 30%; text-align: center; padding: 10px; border-right: 1px solid #eee; }
    .faq-q { font-weight: bold; color: #adb9d3; margin-top: 10px; }
    </style>
...
with col_right:
    ...
    with tab1:
        st.markdown("""<div class="content-box" style="text-align:center;">
            <div class="stat-box"><b>1,240</b><br>Followers</div>
            <div class="stat-box"><b>850</b><br>Check-ins</div>
            <div class="stat-box" style="border:none;"><b>4.9 ⭐</b><br>Rating</div>
        </div>""", unsafe_allow_html=True)
        ...
        st.markdown("""<div class="content-box">
            ✅ All staff are Enhanced DBS checked.<br>
            ✅ First Aid trained leaders on every event.<br>
            ✅ Inclusive activities adapted for all abilities.<br>
        ...
        st.markdown("""<div class="content-box">
            ...
            <p>Yes! We provide all safety gear and specialized equipment for every activity.</p>
            <p class="faq-q">What is the age limit?</p>
            <p>Most events are for ages 12-17, but check individual posters for specific junior sessions.</p>
        </div>""", unsafe_allow_html=True)
            ...
            for i, (img, title) in enumerate(posters):
                with g_cols[i % 5]:
                    if os.path.exists(img):
                        ...
                        if st.button("View", key=f"p_{i}"):
                            st.session_state.photo_index = i
                            st.rerun()
        else:
            l_p, mid, r_p = st.columns([1, 2, 1])
            with mid:
    ...
    with tab3:
        ...
        if st.session_state.booking_step == "select":
            ...
            st.success(f"Booking Registered for {st.session_state.temp_booking['event']}")
        ...
        if not st.session_state.my_bookings:
            ...
            for idx, b in enumerate(st.session_state.my_bookings):
                with st.container():
                    ...
                    if st.button(f"Cancel {b['id']}", key=f"can_{idx}"):
                        st.session_state.my_bookings.pop(idx); st.rerun()
